...
describe('Economy virtual account member autocomplete gate', () => {
  ...
  it('keeps the internal V-Bot User GUID as the canonical value behind the Discord-user payout dropdown', () => {
    ...
    expect(panel).toContain('hint: `Discord ${member.discordId} · GUID ${member.id}`');
  ...
  it('rate-limits payout member lookup and control lookup exposes only human guild members', () => {
