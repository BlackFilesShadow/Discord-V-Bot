...
describe('Goodbye-1 guild-scoped identity and delivery', () => {
  ...
  it('still sends the independent Goodbye when cleanup identity lookup fails', async () => {
    ...
    expect(json.fields?.find(field => field.name === '👤 Mitglied')?.value).toContain(`**Discord:** <@${discordId}>`);
    expect(json.fields?.find(field => field.name.includes('Whitelist'))?.value).toContain('Nicht eindeutig');
  });
});
