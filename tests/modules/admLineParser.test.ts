/**
 * Phase 3, Schritt 1: Golden-Tests fuer den kanonischen ADM-Zeilenparser.
 * Deckt dokumentierte Vanilla-/Konsolen-Varianten ab; Unbekanntes -> UNKNOWN.
 */
import { parseAdmLine, newDateContext, resolveBaseDate } from '../../src/modules/nitrado/adm/admLineParser';
...
describe('admLineParser — Golden', () => {
  ...
  it('PvP-Kill mit (DEAD) und "Player"-Killer: Opfer/Killer korrekt getrennt', () => {
    const ev = parseAdmLine('18:05:30 | Player "Bravo"(DEAD) (id=76561190000000002 pos=<7501.0, 8499.0, 300.0>) killed by Player "Alpha" (id=76561190000000001 pos=<7480.0, 8490.0, 300.0>) with M4A1 from 25.3 meters', ctxWithDate());
    expect(ev?.eventType).toBe('PLAYER_KILLED');
    expect(ev?.actorName).toBe('Bravo'); // Opfer
    expect(ev?.actorPosition).toBe('7501.0, 8499.0, 300.0');
    expect(ev?.targetName).toBe('Alpha'); // Killer
    expect(ev?.targetPosition).toBe('7480.0, 8490.0, 300.0');
    expect(ev?.toolOrWeapon).toBe('M4A1');
    expect(ev?.distanceMeters).toBeCloseTo(25.3);
  });

  it('PvP-Kill OHNE (DEAD) und ohne "Player"-Praefix beim Killer', () => {
  ...
  it('Suizid, NPC-Kill, Placement, Build', () => {
    ...
    const placed = parseAdmLine('18:12:00 | Player "Echo" (id=5 pos=<8,9,10>) placed Fireplace Kit', ctxWithDate());
    expect(placed?.eventType).toBe('PLACEMENT');
    expect(placed?.objectType).toBe('Fireplace Kit');
    const built = parseAdmLine('18:13:00 | Player "Echo" (id=5) built Watchtower Kit', ctxWithDate());
    expect(built?.eventType).toBe('BUILD');
    expect(built?.objectType).toBe('Watchtower Kit');
  });
