...
describe('Gameplay-Feed Embed', () => {
  it('rendert Namen ohne sichtbare Inline-Code-Escapes und ohne Footer/Zeitstempel', () => {
    const embed = buildGameplayFeedEmbed(suicideView(), '#dc2626').toJSON();
    const victim = embed.fields?.find(field => field.name === 'Opfer');
    ...
    expect(victim?.value).toBe('Void\\_\\_Architect');
    expect(victim?.value).not.toContain('`');
  ...
  it('verlinkt DayZ-Koordinaten direkt auf eine iZurvive-Location', () => {
    ...
    const embed = buildGameplayFeedEmbed(suicideView(), '#dc2626').toJSON();
    const position = embed.fields?.find(field => field.name === 'Opfer-Position');
