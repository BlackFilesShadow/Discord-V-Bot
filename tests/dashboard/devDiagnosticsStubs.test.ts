...
jest.mock('../../src/database/prisma', () => ({
  ...
  default: {
    devSession: {
      ...
      count: (...args: unknown[]) => mockDevSessionCount(...args),
    },
    nitradoJob: {
      groupBy: (...args: unknown[]) => mockNitradoGroupBy(...args),
      findMany: (...args: unknown[]) => mockNitradoFindMany(...args),
    },
    gameIdentityLink: {
      groupBy: (...args: unknown[]) => mockLinkGroupBy(...args),
      count: (...args: unknown[]) => mockLinkCount(...args),
    },
    securityEvent: {
      groupBy: (...args: unknown[]) => mockSecurityGroupBy(...args),
      findMany: (...args: unknown[]) => mockSecurityFindMany(...args),
    },
  },
}));
...
jest.mock('../../src/utils/metrics', () => ({
  __esModule: true,
  errorCounter: { get: (...args: unknown[]) => mockMetricGet(...args) },
}));
...
describe('Dashboard-2C scoped DEV stub diagnostics', () => {
    ...
    mockQueryLogRing.mockReturnValue([
      {
        ts: 123,
        level: 'error',
        message: 'Authorization: Bearer abc.secret.token',
        meta: JSON.stringify({ password: 'hunter2', nested: { token: 'secret-token' } }),
      },
    ]);
  ...
  it('bindet Sync-Diagnose an guildIdRestrict und berechnet Economy-Total separat', async () => {
    ...
    mockNitradoGroupBy
      ...
      .mockResolvedValueOnce([{ operation: 'WHITELIST_ADD', _count: { _all: 2 } }]);
    mockNitradoFindMany.mockResolvedValue([
      {
        id: 'job1', guildId, operation: 'WHITELIST_ADD', attempts: 2,
        lastError: 'token=do-not-leak', updatedAt: new Date('2026-08-20T12:00:00.000Z'),
      },
    ]);
    ...
    for (const call of mockNitradoGroupBy.mock.calls) {
      expect(call[0].where).toEqual({ guildId });
    }
