...
jest.mock('../../src/database/prisma', () => ({ __esModule: true, default: { devSession: { findFirst: (...args: unknown[]) => mockDevSessionFindFirst(...args), updateMany: (...args: unknown[]) => mockDevSessionUpdateMany(...args) }, $queryRawUnsafe: (...args: unknown[]) => mockRaw(...args) } }));
jest.mock('../../src/dashboard/clientRegistry', () => ({ __esModule: true, tryGetDashboardClient: jest.fn().mockReturnValue(null), getDashboardClient: jest.fn(), setDashboardClient: jest.fn() }));
jest.mock('../../src/modules/ai/providerStats', () => ({ __esModule: true, getStats: (...args: unknown[]) => mockGetStats(...args) }));
...
describe('Dashboard-2C DEV diagnostics contract', () => {
  it('liefert bei fehlendem Discord-Client einen stabilen vollstaendigen Offline-Shape', async () => {
    ...
    expect(response.body).toMatchObject({ ok: false, statusCode: null, averagePingMs: null, shards: [], cache: { guilds: 0, users: 0, channels: 0 }, user: null });
  });
  ...
  it('redigiert Provider-Fehler auch auf Legacy-Pass-Through-Endpunkten', async () => {
    mockGetStats.mockResolvedValue([{ provider: 'test-provider', configured: true, successCount: 0, failureCount: 2, rateLimitCount: 0, avgLatencyMs: 10, successRate: 0, lastSuccessAt: null, lastFailureAt: new Date(), lastError: 'Authorization: Bearer super-secret-token' }]);
  ...
  it('blockiert AI-Retrieval-Debug ausserhalb des guildIdRestrict vor dem Datenread', async () => {
    const response = await request(makeApp({ guildIdRestrict: '111111111111111111' })).post('/api/v2/dev/status/ai-retrieval-debug').send({ guildId: '222222222222222222', question: 'Was ist aktiv?', limit: 3 });
