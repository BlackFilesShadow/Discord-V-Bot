...
describe('force-link admin command gate', () => {
  it('keeps force-link and force-unlink privileged while only force-unlink remains session-independent', () => {
    ...
    expect(source).toContain('nur die 5-Minuten-Spielzeitregel umgangen');
    expect(source).toContain('der Spielername muss weiterhin eindeutig in den ADM-/Session-Daten erkannt sein');
