...
const matrix = JSON.parse(read('docs/dashboard-mobile-390-matrix.json')) as {
  stage: number;
...
describe('Stage 34 dashboard mobile 390px matrix', () => {
  it('documents 390px viewport inventory', () => {
    expect(matrix.stage).toBe(34);
    expect(matrix.viewport.width).toBe(390);
    expect(matrix.checks.length).toBeGreaterThanOrEqual(5);
    expect(matrix.surfaces.length).toBeGreaterThanOrEqual(6);
  });

  it('binds Playwright evidence for 390px overflow and touch targets', () => {
    expect(mobileSpec).toContain('width: 390');
    expect(mobileSpec).toContain('toBeGreaterThanOrEqual(44)');
    expect(pageMatrix).toContain('width: 390');
