...
export function ZoneEditor({ activeMap, functions, channels, roles, players, zone, onSave, onDelete, saving, deleting }: ZoneEditorProps) {
  ...
  const [circleMode, setCircleMode] = useState<'CREATE' | 'EDIT'>(zone ? 'EDIT' : 'CREATE');
  const [polygonDrawing, setPolygonDrawing] = useState(true);
  const [polygonPreviewPoint, setPolygonPreviewPoint] = useState<{ x: number; y: number }>();
  ...
  useEffect(() => {
    ...
    setCircleMode(zone ? 'EDIT' : 'CREATE');
    setPolygonDrawing(!zone || zone.geometry.type === 'POLYGON');
