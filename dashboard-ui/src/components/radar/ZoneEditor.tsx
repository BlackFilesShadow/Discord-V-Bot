    ...
    <div className="space-y-5" aria-label="Radar-Zoneneditor">
      ...
      <fieldset className="space-y-3 border border-border/70 p-3"><legend className="px-1 text-sm font-medium text-white">Geometrie</legend>
        <div className="grid gap-2 sm:grid-cols-2" role="group" aria-label="Zonenform auswählen">
          <button type="button" aria-pressed={draft.geometry.type === 'CIRCLE'} onClick={() => { update({ geometry: { type: 'CIRCLE', x: 0, y: 0, radiusMeters: 100 } }); setCircleMode('CENTER'); setPolygonPreviewPoint(undefined); }} className={`flex min-h-12 items-center gap-3 border p-3 text-left text-sm ${draft.geometry.type === 'CIRCLE' ? 'border-danger bg-danger/15 text-white' : 'border-border/70 bg-bg-elev/40 text-muted'}`}><CircleDotDashed className="h-5 w-5 shrink-0" aria-hidden="true" /><span><strong className="block text-white">Kreis</strong><span className="text-xs">Mittelpunkt setzen, Radius mit Mauszug bestimmen</span></span></button>
          <button type="button" aria-pressed={draft.geometry.type === 'POLYGON'} onClick={() => { update({ geometry: { type: 'POLYGON', points: [] } }); setPolygonPreviewPoint(undefined); }} className={`flex min-h-12 items-center gap-3 border p-3 text-left text-sm ${draft.geometry.type === 'POLYGON' ? 'border-danger bg-danger/15 text-white' : 'border-border/70 bg-bg-elev/40 text-muted'}`}><MapPin className="h-5 w-5 shrink-0" aria-hidden="true" /><span><strong className="block text-white">Polygon</strong><span className="text-xs">Punkte direkt auf der Karte zeichnen</span></span></button>
        </div>
        <Suspense fallback={<div className="h-[28rem] border border-border/70" aria-label="Zonenkarte wird geladen" />}>
          <DayzRadarMap activeMap={draft.map} zones={[{ id: 'draft', name: draft.name || 'Aktuelle Zone', isActive: draft.isActive, isDraft: true, geometry: draft.geometry }]} onMapClick={setGeometryPoint} onMapMove={point => { if (draft.geometry.type === 'POLYGON') setPolygonPreviewPoint(point); }} focusPoint={circleMode === 'RADIUS' && circle ? circle : undefined} circleCenter={circleMode === 'RADIUS' && circle ? circle : undefined} circleRadiusMode={circleMode === 'RADIUS' && !!circle} onCircleRadiusChange={radiusMeters => circle && update({ geometry: { ...circle, radiusMeters } })} previewPoint={polygon ? polygonPreviewPoint : undefined} />
        </Suspense>
        <p className="text-xs text-muted">{draft.geometry.type === 'CIRCLE' ? circleMode === 'CENTER' ? 'Klicke auf die Karte, um den Kreismittelpunkt zu setzen.' : 'Halte die linke Maustaste gedrückt und ziehe vom Mittelpunkt weg oder zu ihm hin, um den Radius größer oder kleiner zu machen.' : 'Klicke nacheinander auf jeden Eckpunkt. Die rote Linie folgt der Maus bis zum nächsten Punkt und zeigt die Zone präzise Punkt für Punkt.'}</p>
