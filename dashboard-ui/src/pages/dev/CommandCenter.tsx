...
function Diagnostics() {
  ...
  return <Card glow><div className="flex justify-between gap-3"><div><h2 className="font-semibold">Ping / Status / Dev-Eval</h2><p className="text-xs text-muted">WebSocket, Uptime, DB-Roundtrip, Cache, CPU/RAM und Prozessspeicher.</p></div><Button size="sm" variant="ghost" onClick={() => q.refetch()} loading={q.isFetching}>Aktualisieren</Button></div><div className="mt-3">{q.isError ? <p className="text-danger">{errorText(q.error)}</p> : <JsonBlock value={q.data ?? {}}/>}</div></Card>;
}
...
function Admins() {
  ...
  return <Card glow><h2 className="font-semibold mb-3">Admin-Rollen</h2><StepUp reason={reason} setReason={setReason} reAuth={reAuth} setReAuth={setReAuth}/><div className="flex flex-col gap-2 sm:flex-row"><Input value={discordId} onChange={e => setDiscordId(e.target.value)} placeholder="Discord-ID"/><Button size="sm" onClick={() => add.mutate()} loading={add.isPending} disabled={!discordId}>ADMIN hinzufügen</Button></div><div className="space-y-2 mt-4">{(q.data?.items ?? []).map(a => <div key={a.id} className="flex flex-col items-start justify-between gap-2 border border-border rounded-md p-2 sm:flex-row sm:items-center"><span><b>{a.username}</b> <span className="text-muted text-xs break-all">{a.discordId} · {a.role}</span></span>{a.role === 'ADMIN' && <Button size="sm" variant="danger" onClick={() => remove.mutate(a.discordId)} loading={remove.isPending}>Entfernen</Button>}</div>)}</div></Card>;
}
