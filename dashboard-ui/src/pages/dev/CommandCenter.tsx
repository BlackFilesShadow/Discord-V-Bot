...
function SecurityTools() {
  const qc = useQueryClient(); const toast = useToast();
  const [ip, setIp] = useState(''); const [listType, setListType] = useState('BLACKLIST'); const [listReason, setListReason] = useState(''); const [hours, setHours] = useState('0'); const [eventId, setEventId] = useState(''); const [reason, setReason] = useState('Security Änderung'); const [reAuth, setReAuth] = useState('');
  const q = useQuery({ queryKey: [base, 'security'], queryFn: () => api.get<Record<string, unknown>>(`${base}/security`) });
  const setIp = useMutation({ mutationFn: () => api.put(`${base}/security/ip/${encodeURIComponent(ip)}`, { listType, listReason, durationHours: Number(hours), reason, reAuth }), onSuccess: () => { toast.success('IP-Liste aktualisiert'); void qc.invalidateQueries({ queryKey: [base, 'security'] }); }, onError: e => toast.error('Fehler', errorText(e)) });
  ...
  return <div className="space-y-4"><Card glow><h2 className="font-semibold mb-3">IP Black-/Whitelist & Event-Auflösung</h2><StepUp reason={reason} setReason={setReason} reAuth={reAuth} setReAuth={setReAuth}/><div className="grid gap-2 md:grid-cols-4"><Input value={ip} onChange={e => setIp(e.target.value)} placeholder="IPv4 / IPv6"/><Select value={listType} onChange={e => setListType(e.target.value)}><option value="BLACKLIST">BLACKLIST</option><option value="WHITELIST">WHITELIST</option></Select><Input value={listReason} onChange={e => setListReason(e.target.value)} placeholder="Listen-Begründung"/><Input value={hours} onChange={e => setHours(e.target.value)} type="number" placeholder="Stunden (0=permanent)"/></div><div className="flex gap-2 mt-2"><Button size="sm" onClick={() => setIp.mutate()} disabled={!ip}>IP setzen</Button><Button size="sm" variant="danger" onClick={() => removeIp.mutate()} disabled={!ip}>IP entfernen</Button></div><div className="flex gap-2 mt-4"><Input value={eventId} onChange={e => setEventId(e.target.value)} placeholder="SecurityEvent-ID"/><Button size="sm" onClick={() => resolve.mutate()} disabled={!eventId}>Event als gelöst markieren</Button></div></Card><Card><JsonBlock value={q.data ?? {}}/></Card></div>;
}
