  ...
  return (
    <div className="space-y-4 min-w-0">
      <Card glow>
        ...
        <Button variant="ghost" size="sm" onClick={refreshAll} disabled={loading || adm.loading}>
          <RefreshCw className={`h-3.5 w-3.5 mr-1 ${loading || adm.loading ? 'animate-spin' : ''}`} /> Aktualisieren
        </Button>
          ...
          <Card>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3 text-xs">
              {(['PENDING', 'RUNNING', 'DONE', 'FAILED', 'DEAD'] as const).map(k => (
                <div key={k} className="rounded-md border border-border/30 px-2 py-1.5 min-w-0">
                  <div className={`text-[10px] uppercase ${STATUS_COLORS[k]}`}>{k}</div>
                  <div className="text-base font-semibold break-words">{data.counts[k] ?? 0}</div>
                </div>
              ))}
            </div>
              ...
              <table className="w-full min-w-[760px] text-xs">
                ...
                <tbody>
                  ...
                  {data.recentFailures.map(f => (
                    <tr key={f.id} className="border-t border-border/20 align-top">
                      <td className="py-1 font-mono break-all">{f.operation}</td>
                      ...
                      <td className="text-danger break-all max-w-[420px]">{f.lastError ?? '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <Card>
        ...
        {adm.error && (
          <div role="alert" className="text-xs text-danger flex gap-2 mb-2 break-words"><AlertTriangle className="h-3.5 w-3.5 mt-0.5 shrink-0" /> {adm.error}</div>
        )}
        {adm.data && (
          <>
                  ...
                  {adm.data.connections.map(c => {
                    ...
                    return (
                      <tr key={c.nitradoConnId} className="border-t border-border/20 align-top">
                        <td className="py-1 break-all">{c.alias} <span className="text-muted font-mono">({c.alias5})</span></td>
                        <td>{c.slot}</td>
                        <td className="font-mono break-all max-w-[300px]">
                          {c.source
                        ...
                        <td className="font-mono break-all max-w-[260px]">{c.cursor?.fileName ?? <span className="text-muted">—</span>}</td>
