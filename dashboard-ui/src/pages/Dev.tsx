  ...
  return (
    <nav aria-label="DEV Tools" className="space-y-4">
      ...
      <div className="relative">
        ...
        <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Tools suchen…" className="input-premium w-full rounded-md text-xs text-white pl-8 pr-7 py-1.5 placeholder:text-muted/70 focus:outline-none" aria-label="Tools suchen" />
        {query && <button type="button" onClick={() => setQuery('')} className="absolute right-1.5 top-1/2 -translate-y-1/2 text-muted hover:text-white p-0.5 rounded focus-ring" aria-label="Suche leeren"><XIcon className="h-3 w-3" /></button>}
      </div>
          ...
          <div key={group.cat} className="space-y-1">
            ...
            <ul className="space-y-0.5">
              {group.items.map(t => {
                ...
                return <li key={t.slug} className="group"><NavLink to={to} className={['relative flex items-center gap-2.5 px-2.5 py-1.5 rounded-md text-xs transition-colors focus-ring', isActive ? 'bg-accent/15 text-white border border-accent/30' : 'text-muted hover:text-white hover:bg-bg-elev/60 border border-transparent'].join(' ')}><Icon className="h-3.5 w-3.5 shrink-0"/><span className="truncate flex-1">{t.label}</span>{t.status === 'stub' && <span className="text-[9px] uppercase tracking-wider text-muted/70">soon</span>}<button type="button" onClick={e => { e.preventDefault(); e.stopPropagation(); toggle(t.slug); }} className={['p-0.5 rounded focus-ring transition-colors', isP ? 'text-accent opacity-100' : 'text-muted opacity-0 group-hover:opacity-100 hover:text-accent'].join(' ')} aria-label={isP ? `${t.label} entpinnen` : `${t.label} pinnen`}><Star className={`h-3 w-3 ${isP ? 'fill-current' : ''}`}/></button></NavLink></li>;
              })}
            </ul>
          </div>
        ))}
      ...
      <div className="pt-3 border-t border-border"><Button size="sm" variant="ghost" className="w-full justify-start text-muted hover:text-danger" onClick={() => { void dev.logout(); }}><LogOut className="h-3.5 w-3.5" /> DEV-Logout</Button></div>
    </nav>
  );
}
