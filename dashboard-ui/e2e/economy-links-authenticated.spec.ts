...
async function stub(page: Page, forceFailure = false) {
  ...
  await page.route('**/api/v2/**', async route => {
    ...
    if (path === `/api/v2/guilds/${GUILD_ID}/economy-links` && method === 'GET') return json(route, {
      links: [{ userDiscordId: LINKED_USER, status: 'VERIFIED', verifiedAt: '2026-08-19T08:00:00.000Z' }],
    });
    ...
    if (path === `/api/v2/guilds/${GUILD_ID}/economy-links/grant` && method === 'POST') {
      return forceFailure ? json(route, { error: 'LINK_CONFLICT' }, 409) : json(route, { ok: true });
    }
...
test.describe('Authenticated Economy-Links', () => {
  test('List, Force-Link und Unlink bleiben exakt Guild+Slot-gescoped', async ({ page }) => {
    ...
    await page.getByPlaceholder('Game-ID').fill('PlayerGuid-ABC');
    ...
    expect(find(mutations, 'POST', `/api/v2/guilds/${GUILD_ID}/economy-links/grant`)).toMatchObject({
      query: `?slot=${SLOT}`,
      body: { userDiscordId: NEW_USER, gameId: 'PlayerGuid-ABC' },
    });
  ...
  test('Client-Validierung blockiert ungültige Discord-ID und zu kurze Game-ID', async ({ page }) => {
    const mutations = await stub(page);
    await page.goto(`/servers/${GUILD_ID}/server/${SLOT}?tab=links`);
    await page.getByPlaceholder('Discord-ID').fill('123');
    await page.getByPlaceholder('Game-ID').fill('ab');
...
for (const width of [320, 360, 375, 390, 430] as const) {
  test(`${width}px Economy-Links bleibt ohne Seiten-Overflow`, async ({ page }) => {
    await stub(page);
