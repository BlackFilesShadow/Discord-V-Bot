...
function feed(kind: FeedKind, overrides: Record<string, unknown> = {}) {
  const categories = kind === 'KILL'
    ? ['PVP']
    : kind === 'DEATH'
      ? ['SUICIDE', 'NPC', 'VEHICLE', 'OTHER']
      : kind === 'BUILD'
        ? ['BUILD', 'DISMANTLE', 'DESTROY']
        : kind === 'PLACEMENT'
          ? ['PLACEMENT']
          : kind === 'FLAG'
            ? ['RAISED']
  ...
  return {
    ...
    showTargetCoords: kind === 'KILL' || kind === 'FLAG',
    showTool: kind !== 'FLAG' && kind !== 'PLACEMENT',
    showDistance: kind === 'KILL',
    embedColor: '#dc2626',
    lastEventAt: '2026-09-09T09:00:00.000Z',
    lastPolledAt: '2026-09-09T09:01:00.000Z',
    lastErrorMsg: null,
    openDeliveryCount: 0,
    retryDeliveryCount: 0,
    failedDeliveryCount: 0,
    oldestOpenAt: null,
    lastSuccessAt: '2026-09-09T09:00:05.000Z',
...
async function stub(page: Page, opts: { saveFailure?: boolean; listFailure?: boolean; actionDelayMs?: number } = {}) {
  ...
  await page.route('**/api/v2/**', async route => {
    ...
    if (path === `/api/v2/guilds/${GUILD_ID}/killfeed` && method === 'GET') {
      ...
      const kind = (url.searchParams.get('kind') ?? 'DEATH') as FeedKind;
      return json(route, {
        kind,
        configs: [feed(kind, kind === 'PLAYER_LIST' ? {
          showActorCoords: false,
          lastPlayerCount: 3,
          lastPlayerListAt: '2026-09-09T09:01:00.000Z',
          playerListIntervalMinutes: 15,
          nextPlayerListPostAt: '2026-09-09T09:15:00.000Z',
        } : {})],
      });
    }
    ...
    if (path === `/api/v2/guilds/${GUILD_ID}/killfeed` && method === 'POST') {
      const kind = (url.searchParams.get('kind') ?? 'DEATH') as FeedKind;
      return opts.saveFailure ? json(route, { error: 'FEED_SAVE_FAILED' }, 409) : json(route, feed(kind, { id: 'feed-new' }), 201);
    }
    if (path === `/api/v2/guilds/${GUILD_ID}/killfeed/${CONFIG_ID}` && method === 'PATCH') {
      ...
      const kind = (url.searchParams.get('kind') ?? 'DEATH') as FeedKind;
      return opts.saveFailure ? json(route, { error: 'FEED_SAVE_FAILED' }, 409) : json(route, feed(kind));
    }
...
test.describe('Authenticated Killfeed & Deathfeed separation', () => {
  test('Killfeed ist eigenständig anklickbar und CRUD bleibt exakt Guild+Slot+KILL-gescoped', async ({ page }) => {
    ...
    await expect(page.getByText('Killfeed ist ausschließlich PvP.')).toBeVisible();
    await expect(page.getByText('💀 V-Kill Report')).toBeVisible();
    ...
    expect(find(mutations, 'PATCH', `/api/v2/guilds/${GUILD_ID}/killfeed/${CONFIG_ID}`)).toMatchObject({
      query: `?slot=${SLOT}&kind=KILL`,
      body: { isActive: false },
    });

    await page.getByRole('button', { name: /Neu/ }).click();
    const createDialog = page.getByRole('dialog');
    await expect(createDialog.getByRole('heading', { name: '💀 Killfeed konfigurieren' })).toBeVisible();
    ...
    expect(edit).toMatchObject({ query: `?slot=${SLOT}&kind=KILL`, body: expect.objectContaining({ embedColor: '#112233' }) });
    ...
    expect(find(mutations, 'DELETE', `/api/v2/guilds/${GUILD_ID}/killfeed/${CONFIG_ID}`)?.query).toBe(`?slot=${SLOT}&kind=KILL`);
  });

  test('Deathfeed ist separat anklickbar und enthält kein PVP', async ({ page }) => {
    const mutations = await stub(page);
    await page.goto(`/servers/${GUILD_ID}/server/${SLOT}?tab=killfeed`);
    await page.getByRole('button', { name: '☠️ Deathfeed' }).click();

    await expect(page.getByText('Deathfeed ist vom PvP-Killfeed getrennt.')).toBeVisible();
    await expect(page.getByText('🩸 Self Kill Report')).toBeVisible();
    await expect(page.getByText('☣️ Wild Kill Report')).toBeVisible();
    await expect(page.getByText('💥 Crash Kill Report')).toBeVisible();
    await expect(page.getByText('☠️ Death Report')).toBeVisible();
    await expect(page.getByText('💀 V-Kill Report')).toHaveCount(0);

    await page.getByRole('button', { name: /Neu/ }).click();
    const dialog = page.getByRole('dialog');
    await expect(dialog.getByRole('heading', { name: '☠️ Deathfeed konfigurieren' })).toBeVisible();
    await dialog.getByLabel('Discord-Channel').selectOption(CHANNEL_ID);
    await dialog.getByRole('button', { name: 'Speichern' }).click();

    await expect.poll(() => mutations.find(row => row.method === 'POST' && row.query.includes('kind=DEATH'))).toBeTruthy();
    expect(mutations.find(row => row.method === 'POST' && row.query.includes('kind=DEATH'))).toMatchObject({
      body: expect.objectContaining({
        categories: ['SUICIDE', 'NPC', 'VEHICLE', 'OTHER'],
        showActorCoords: true,
  ...
  test('Editor validiert Killfeed lokal und Backend-Save-Fehler bleibt sichtbar', async ({ page }) => {
    ...
    await dialog.getByRole('button', { name: 'Speichern' }).click();
    await expect(dialog.getByText('Channel, gültige Farbe und ausschließlich PVP sind erforderlich.')).toBeVisible();
  ...
  test('synchroner Doppel-Klick erzeugt nur eine Killfeed-Toggle-Mutation', async ({ page }) => {
    const mutations = await stub(page, { actionDelayMs: 200 });
    await page.goto(`/servers/${GUILD_ID}/server/${SLOT}?tab=killfeed`);

    const toggle = page.getByRole('button', { name: 'Killfeed deaktivieren' });
...
for (const width of [320, 360, 375, 390, 430] as const) {
  test(`${width}px Killfeed bleibt mit eigenem Deathfeed-Button ohne Seiten-Overflow`, async ({ page }) => {
    await stub(page);
    await page.setViewportSize({ width, height: 900 });
    await page.goto(`/servers/${GUILD_ID}/server/${SLOT}?tab=killfeed`);
    await expect(page.getByRole('heading', { name: 'Nitrado Gameplay-Feeds' })).toBeVisible();
    await expect(page.getByRole('button', { name: '💀 Killfeed' })).toBeVisible();
    await expect(page.getByRole('button', { name: '☠️ Deathfeed' })).toBeVisible();
    await page.getByRole('button', { name: /Neu/ }).click();
