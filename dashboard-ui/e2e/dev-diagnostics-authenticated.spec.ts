...
async function stubDevShell(page: Page): Promise<void> {
  ...
  await page.route('**/api/v2/dev/status', route => json(route, { active: true, eligible: true, expiresAt: '2026-08-20T20:00:00.000Z' }));
...
async function stubDiagnostics(page: Page): Promise<{ denyDatabase: () => void }> {
  ...
  await page.route('**/api/v2/dev/status/database', route => {
    if (databaseDenied) return json(route, { error: 'Keine globale DEV-Berechtigung.', code: 'DEV_IDENTITY_REQUIRED' }, 403);
    return json(route, {
      ok: true,
      degraded: false,
      pingMs: 4,
  ...
  await page.route('**/api/v2/dev/status/nitrado', route => json(route, {
    ...
    queryMs: 3,
    recentFailures: [{
      id: 'job1', operation: 'WHITELIST_ADD_WITH_A_VERY_LONG_OPERATION_NAME', guildId: '111111111111111111', status: 'FAILED', attempts: 2,
      lastError: 'Ein sehr langer bereits redigierter Diagnosefehler '.repeat(8), updatedAt: '2026-08-20T12:00:00.000Z',
    }],
  ...
  await page.route('**/api/v2/dev/status/adm', route => json(route, {
    ...
    queryMs: 5,
    connections: [{
      nitradoConnId: 'conn1', guildId: '111111111111111111', slot: 1, alias: 'SehrLangerServerAliasFuerMobile', alias5: 'slot1', serviceId: '1234567', admLinked: true,
      source: { profileDir: '/gameserver/profile/'.repeat(12), source: 'nitrado', timeZone: 'Europe/Berlin', lastVerifiedAt: '2026-08-20T12:00:00.000Z', lastError: null, updatedAt: '2026-08-20T12:00:00.000Z' },
      cursor: { fileName: 'DayZServer_X1_x64_very_long_adm_filename.ADM', lastModifiedAt: 1, lastModifiedIso: '2026-08-20T12:00:00.000Z', lastKnownSize: 999999, processedByteOffset: 888888, lastSuccessAt: '2026-08-20T12:00:00.000Z', lastError: null, updatedAt: '2026-08-20T12:00:00.000Z' },
    }],
  }));
...
test('malformed erfolgreicher Snapshot wird clientseitig fail-closed verworfen', async ({ page }) => {
  await stubDevShell(page);
  await page.route('**/api/v2/dev/snapshot', route => json(route, { ...GOOD_SNAPSHOT, guildCount: -1, memory: { ...GOOD_SNAPSHOT.memory, heapUsed: 100, heapTotal: 10 } }));
  await page.goto('/dev/bot-status');
