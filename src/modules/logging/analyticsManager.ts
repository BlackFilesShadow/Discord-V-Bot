...
export async function checkAlerts(): Promise<string[]> {
  ...
  if (unresolvedCritical > 0) {
    alerts.push(`ðŸ”´ ${unresolvedCritical} ungelöste kritische Security-Events`);
  }
  ...
  if (quarantined > 0) {
    alerts.push(`ðŸŸ¡ ${quarantined} Pakete in Quarantäne`);
  }
  ...
  if (pendingDeletions > 0) {
    alerts.push(`ðŸŸ¡ ${pendingDeletions} ausstehende DSGVO-Löschanträge`);
  }
