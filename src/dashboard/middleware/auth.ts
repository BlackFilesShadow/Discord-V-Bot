...
/**
 * Stage 36: every authenticated dashboard request must be bound to the
 * persistent application Session row created by the OAuth callback. A cookie
 * carrying userId/discordId without sessionToken is stale/legacy state and must
 * fail closed; otherwise revocation/expiry can be bypassed after partial
 * session restoration.
 */
 ...
 *
 * Strikt pro Guild — Grants aus anderen Guilds zaehlen NIE. Fuer Nicht-Owner
 * ist eine aktuelle Discord-Mitgliedschaft zwingend; stale DB-Grants allein
 * koennen keinen Guild-Zugriff herstellen.
 */
 ...
 *
 * P0-Compliance:
 *   - MFA: OPTIONAL. Standardmaessig AUS. 2FA wird nur erzwungen, wenn
 *     `DEV_REQUIRE_MFA=true` explizit gesetzt ist. Ohne diese Variable reicht
 *     ein korrektes DEV_PASSWORD + aktive DevSession.
...
export async function requireDev(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  if (!mfaRequired && !warnedAboutDisabledDevMfa) {
    ...
    logger.warn('[DEV] DEV_REQUIRE_MFA ist nicht aktiv — DEV-Konsole laeuft mit Passwort-only-Schutz (kein 2FA-Zwang). Fuer Produktion DEV_REQUIRE_MFA=true setzen.');
  }
  ...
  if (!mfa.ok) {
    ...
    res.status(403).json({
      error: 'DEV_REQUIRE_MFA ist aktiv. Bitte 2FA im Profil aktivieren, um den DEV-Bereich zu nutzen.',
