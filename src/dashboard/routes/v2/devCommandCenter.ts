 ...
 * ausgenommen). `v2.ts` legt vor diesen Router bereits globale Developer-
 * Identitaet, aktive DevSession, Input-Guard und den kanonischen verifizierten
...
function requireGuildScope(req: Parameters<typeof requireDev>[0], res: Parameters<typeof requireDev>[1], guildId: string): boolean {
  ...
  if (restricted && restricted !== guildId) {
    res.status(403).json({ error: 'DEV-Session ist auf eine andere Guild beschrÃ¤nkt.', code: 'DEV_GUILD_SCOPE_DENIED' });
    return false;
  }
  return true;
}
...
function safeConfigValue(key: string, value: unknown): unknown {
  return /password|secret|token|api[_.-]?key|rcon|credential|private/i.test(key) ? 'Â«redigiertÂ»' : value;
}
...
// /admin-config (historisch devOnly) -----------------------------------------
...
devCommandCenterRouter.put('/config/:key', async (req, res) => {
  ...
  if (PROTECTED_PREFIXES.some(p => key.toLowerCase().startsWith(p)) || !ALLOWED_CONFIG_KEYS.has(key)) { res.status(403).json({ error: 'SchlÃ¼ssel ist geschÃ¼tzt oder nicht freigegeben.' }); return; }
...
devCommandCenterRouter.delete('/config/:key', async (req, res) => {
  ...
  if (existing.category === 'system') { res.status(403).json({ error: 'System-Konfiguration ist geschÃ¼tzt.' }); return; }
  await prisma.botConfig.delete({ where: { key } });
  logDevAction('CONFIG_DELETED', req, { key, reason: String(req.body.reason) });
  res.json({ deleted: true });
});

// /admin-security (historisch devOnly) ---------------------------------------
...
devCommandCenterRouter.put('/security/ip/:ip', async (req, res) => {
  ...
  if (isIP(ip) === 0) { res.status(400).json({ error: 'UngÃ¼ltige IPv4/IPv6-Adresse.' }); return; }
  ...
  if (!reasonText) { res.status(400).json({ error: 'BegrÃ¼ndung fehlt.' }); return; }
...
devCommandCenterRouter.delete('/security/ip/:ip', async (req, res) => {
  const ip = decodeURIComponent(String(req.params.ip)).trim();
  if (isIP(ip) === 0) { res.status(400).json({ error: 'UngÃ¼ltige IP.' }); return; }
...
// /xp-config (historisch devOnly; strikt pro Guild) --------------------------
...
devCommandCenterRouter.patch('/xp/:guildId', async (req, res) => {
  ...
  if (!SNOWFLAKE.test(guildId)) { res.status(400).json({ error: 'UngÃ¼ltige guildId.' }); return; }
  ...
  if (!intField('messageXpMin', 0, 10000) || !intField('messageXpMax', 0, 10000) || !intField('voiceXpPerMinute', 0, 10000) || !intField('maxLevel', 1, 100)) {
    res.status(400).json({ error: 'UngÃ¼ltiger XP-Zahlenwert.' }); return;
  }
  ...
  if (effMin > effMax) { res.status(400).json({ error: 'Min-XP darf nicht grÃ¶ÃŸer als Max-XP sein.' }); return; }
  if (req.body?.maxLevelRoleId !== undefined) {
    const v = req.body.maxLevelRoleId;
    if (v !== null && !SNOWFLAKE.test(String(v))) { res.status(400).json({ error: 'UngÃ¼ltige maxLevelRoleId.' }); return; }
    data.maxLevelRoleId = v === null ? null : String(v);
  }
  if (req.body?.allowedRoleIds !== undefined) {
    if (!Array.isArray(req.body.allowedRoleIds) || !req.body.allowedRoleIds.every((x: unknown) => typeof x === 'string' && SNOWFLAKE.test(x))) { res.status(400).json({ error: 'allowedRoleIds ungÃ¼ltig.' }); return; }
    data.allowedRoleIds = [...new Set(req.body.allowedRoleIds as string[])];
  }
  if (req.body?.allowedChannelIds !== undefined) {
    if (!Array.isArray(req.body.allowedChannelIds) || !req.body.allowedChannelIds.every((x: unknown) => typeof x === 'string' && SNOWFLAKE.test(x))) { res.status(400).json({ error: 'allowedChannelIds ungÃ¼ltig.' }); return; }
...
devCommandCenterRouter.put('/xp/:guildId/level-role/:level', async (req, res) => {
  ...
  if (!SNOWFLAKE.test(guildId) || !Number.isInteger(level) || level < 1 || level > 1000 || !SNOWFLAKE.test(roleId)) { res.status(400).json({ error: 'UngÃ¼ltige Guild/Level/Rolle.' }); return; }
  if (!requireGuildScope(req, res, guildId)) return;
...
devCommandCenterRouter.delete('/xp/:guildId/level-role/:level', async (req, res) => {
  const guildId = String(req.params.guildId);
  const level = Number(req.params.level);
  if (!SNOWFLAKE.test(guildId) || !Number.isInteger(level)) { res.status(400).json({ error: 'UngÃ¼ltige Guild/Level.' }); return; }
  if (!requireGuildScope(req, res, guildId)) return;
