...
function hardenPassThroughPayload(path: string, body: unknown): unknown {
  ...
  if (path === '/nitrado') {
    ...
    return { ...obj, recentFailures: failures.map(row => {
  ...
  if (path === '/adm') {
    ...
    return { ...obj, connections: connections.map(row => {
      if (!row || typeof row !== 'object' || Array.isArray(row)) return row;
      const item = row as Record<string, unknown>;
      const source = item.source && typeof item.source === 'object' && !Array.isArray(item.source) ? item.source as Record<string, unknown> : null;
      const cursor = item.cursor && typeof item.cursor === 'object' && !Array.isArray(item.cursor) ? item.cursor as Record<string, unknown> : null;
      return {
        ...item,
...
interface TimedResult<T> { value: T | null; ms: number; error: string | null; }

async function timed<T>(fn: () => Promise<T>): Promise<TimedResult<T>> {
  const startedAt = Date.now();
  try { return { value: await fn(), ms: Date.now() - startedAt, error: null }; }
  catch (error) { return { value: null, ms: Date.now() - startedAt, error: redactDiagnosticText((error as Error).message) }; }
}
...
devDiagnosticsContractRouter.get('/database', requireDev, async (_req, res) => {
  ...
  const tables = await timed(() => prisma.$queryRawUnsafe<Array<{ relname: string; n_live_tup: bigint; n_dead_tup: bigint }>>(`
    SELECT relname, n_live_tup, n_dead_tup FROM pg_stat_user_tables ORDER BY n_live_tup DESC LIMIT 25
  `));
  ...
  const conns = await timed(() => prisma.$queryRawUnsafe<Array<{ state: string | null; count: bigint }>>(`
    SELECT state, count(*)::bigint AS count FROM pg_stat_activity WHERE datname = current_database() GROUP BY state
  `));
  ...
  const errors = { ping: ping.error, tables: tables.error, size: dbSize.error, connections: conns.error, migrations: migrations.error };
  ...
  res.json({
    ...
    topTables: (tables.value ?? []).map(row => ({ name: row.relname, liveRows: Number(row.n_live_tup), deadRows: Number(row.n_dead_tup) })),
    errors,
...
devDiagnosticsContractRouter.get('/discord', requireDev, (_req, res) => {
  ...
  if (!client) {
    res.json({ ok: false, error: 'Discord-Client nicht gebunden.', statusCode: null, averagePingMs: null, shards: [], cache: { guilds: 0, users: 0, channels: 0 }, user: null, generatedAt: new Date().toISOString() });
    return;
  }
  ...
  res.json({
    ...
    shards: Array.from(client.ws.shards.values()).map(shard => ({ id: shard.id, status: shard.status, pingMs: shard.ping })),
    cache: { guilds: client.guilds.cache.size, users: client.users.cache.size, channels: client.channels.cache.size },
...
devDiagnosticsContractRouter.post('/ai-retrieval-debug', requireDev, async (req, res) => {
  const body = req.body;
  if (!body || typeof body !== 'object' || Array.isArray(body)) { res.status(400).json({ error: 'Ungueltiger Request-Body.' }); return; }
  const record = body as Record<string, unknown>;
  if (typeof record.guildId !== 'string' || !/^\d{17,20}$/.test(record.guildId.trim())) { res.status(400).json({ error: 'Ungueltige guildId.' }); return; }
  if (typeof record.question !== 'string') { res.status(400).json({ error: 'Ungueltige Frage.' }); return; }
  const guildId = record.guildId.trim();
  const question = record.question.trim();
  if (question.length < 2 || question.length > 4000) { res.status(400).json({ error: 'Frage muss 2 bis 4000 Zeichen lang sein.' }); return; }
  let limit = 3;
  if (record.limit !== undefined) {
    if (typeof record.limit !== 'number' || !Number.isInteger(record.limit) || record.limit < 1 || record.limit > 10) { res.status(400).json({ error: 'limit muss eine Ganzzahl zwischen 1 und 10 sein.' }); return; }
    limit = record.limit;
  }
  const restrict = req.devSession?.scope.guildIdRestrict ?? null;
  if (restrict && restrict !== guildId) { res.status(403).json({ error: 'DEV-Session ist auf eine andere Guild beschraenkt.', code: 'DEV_SCOPE_RESTRICTED' }); return; }
  try {
