...
CREATE TABLE "NitradoRestartPlan" (
    ...
    CONSTRAINT "NitradoRestartPlan_interval_check"
      CHECK ("intervalHours" IS NULL OR "intervalHours" IN (1, 2, 3, 4, 6, 8, 12, 24)),
    CONSTRAINT "NitradoRestartPlan_start_time_check"
      CHECK ("startTime" IS NULL OR "startTime" ~ '^(?:[01][0-9]|2[0-3]):[0-5][0-9]$'),
    CONSTRAINT "NitradoRestartPlan_times_check"
      CHECK (jsonb_typeof("times") = 'array' AND jsonb_array_length("times") <= 24),
